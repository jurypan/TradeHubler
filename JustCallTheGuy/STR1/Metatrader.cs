...
namespace JustCallTheGuy.STR1
{
    public class Metatrader
    {
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "get", "post")] HttpRequestData req)
        {
            ...
            // Log item
            _logger.LogInformation($"Strategy1_Metatrader || Request body : {requestBody}");
            ...
            {
                // Parse object
                var mt = Str1MetatraderRequest.Parse(requestBody);

                // Log item
                _logger.LogDebug($"Strategy1_Metatrader || Parsed Metatrader object : AccountID={mt.AccountID}, Instrument={mt.Instrument}, ClientID={mt.ClientID}, Price={mt.Price}, TradingviewTicker={mt.TradingviewTicker},", requestBody);

                // Get TradingviewAlert from the database
                var tvAlert = await _dbContext.TradingviewAlert.Include(f => f.Trades).FirstOrDefaultAsync(f => f.AccountID == mt.AccountID && f.Instrument.Equals(mt.TradingviewTicker) && (f.Trades.Count == 0 || f.Trades.Any(g => g.ClientID == mt.ClientID && g.Executed == false)) && f.StrategyType == StrategyType.Strategy1);

                // If ther eis not tradingview tvAlert in the db -> return OK
                if (tvAlert == null)
                {
                    // Log item
                    _logger.LogInformation("Strategy1_Metatrader || Tradingview Alert not found");
                ...
                if (trade == null)
                {
                    ...
                    // Log item
                    _logger.LogInformation($"Strategy1_Metatrader || Metatrader trade not found, created in the database with ID : {trade.ID}", trade);
                }
                ...
                {
                    // Log item
                    _logger.LogWarning($"Strategy1_Metatrader || BUY order is send to Metatrader : BUY,tp={tvAlert.TakeProfit - trade.Offset},sl={tvAlert.StopLoss - trade.Offset}", trade);
                ...
                {
                    // Log item
                    _logger.LogInformation("Strategy1_Metatrader || NONE order is send to Metatrader : NONE,tp=0.0,sl=0.0,comment=''");
